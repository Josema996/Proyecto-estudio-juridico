...
export default function ClienteFormPage() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      <button onClick={() => navigate('/clientes')} className="flex items-center gap-2 text-slate-500 hover:text-slate-700 mb-6 text-sm font-medium transition-colors">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">{isEdit ? 'Editar cliente' : 'Nuevo cliente'}</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-100 shadow-card p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Email" type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="cliente@email.com" className="col-span-2" />
          <Input label="Dirección" value={form.direccion} onChange={e => set('direccion', e.target.value)} placeholder="Av. Corrientes 1500, CABA" className="col-span-2" />
        </div>
