...
export default function DocumentosPage() {
  ...
  const UploadZone = (
    <div
      ...
      onDrop={handleDrop}
      className={cn(
        'border-2 border-dashed rounded-2xl p-6 mb-4 text-center transition-all duration-200',
        dragging
          ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/20'
          : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-slate-300 dark:hover:border-slate-600'
      )}
    >
      <Upload className={cn('w-7 h-7 mx-auto mb-2 transition-colors', dragging ? 'text-primary-500' : 'text-slate-300 dark:text-slate-600')} />
      <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">
        {dragging ? 'Soltá el archivo aquí' : isMobile ? 'Tocá para seleccionar un archivo' : 'Arrastrá un archivo o hacé clic para seleccionar'}
      </p>
      <p className="text-xs text-slate-400 dark:text-slate-500 mb-4">PDF, Word, Excel, imágenes — hasta 50 MB</p>

      <div className={cn('flex gap-3 flex-wrap justify-center', isMobile ? 'flex-col items-stretch' : 'items-center')}>
        <Select value={selectedCausa} onChange={e => setSelectedCausa(e.target.value)} className={isMobile ? 'w-full' : 'w-64 text-left'}>
          <option value="">Sin causa vinculada</option>
          {mockCausas.map(c => (
            <option key={c.id} value={c.id}>{c.numero_expediente} — {c.caratula.slice(0, 40)}</option>
          ))}
        </Select>
        <label className="cursor-pointer bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2">
      ...
      <div>
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <Select value={causaFilter} onChange={e => setCausaFilter(e.target.value)} className="flex-1">
              <option value="">Todos los documentos</option>
              {mockCausas.map(c => (
                <option key={c.id} value={c.id}>{c.numero_expediente} — {c.caratula.slice(0, 30)}</option>
              ))}
            </Select>
            <span className="text-xs text-slate-400 flex-shrink-0">{filtered.length} doc{filtered.length !== 1 ? 's' : ''}</span>
          </div>

          {/* Lista cards mobile */}
          {filtered.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 py-16 text-center">
              <FolderOpen className="w-8 h-8 text-slate-200 dark:text-slate-700 mx-auto mb-2" />
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-card overflow-hidden">
              {filtered.map((doc, i) => {
                ...
                return (
                  <div key={doc.id} className={`flex items-center gap-3 px-4 py-3.5 ${i < filtered.length - 1 ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}>
                    <div className="flex-shrink-0">
                      <FileIcon mime={doc.mime_type} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-slate-800 dark:text-slate-200 text-sm truncate">{doc.nombre}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <FileTypeBadge mime={doc.mime_type} />
                        <span className="text-xs text-slate-400">{formatSize(doc.size_bytes)}</span>
                        {causa && <span className="text-xs font-mono text-primary-500 truncate">{causa.numero_expediente}</span>}
                      </div>
                    </div>
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      <button onClick={() => setConfirmId(doc.id)} className="p-2 text-slate-400 hover:text-red-500 rounded-lg transition-colors">
                  ...
                  <tr key={doc.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => setConfirmId(doc.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:text-red-400 rounded-lg transition-colors">
